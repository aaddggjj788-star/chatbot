    ...
    try {
      ...
      if (
        ...
        generationInstruction = [
          'ユーザーから送られた内容を確認したところ、送り返す言葉が1つ不足しているケースです。',
          '',
          '鑑定士本人がユーザーへ直接話しかける、柔らかく自然な文章にしてください。',
          'システム処理や判定結果を説明しているような文章にしてはいけません。',
          '',
          '「一工程」「必要工程」「不足分」「補完」「要素」など、事務的・システム的な表現は使わないでください。',
          '',
          '「先ほどお送り頂いた内容を確認しましたが、一つ足りていない言葉がありました」のように、',
          '実際の会話として自然な伝え方をしてください。',
          '',
          '今回は不足していた1つについては鑑定士側で対応し、このまま先へ進めることを伝えてください。',
          'ただし「こちらで補完しました」のような機械的な言い方は避けてください。',
          '',
          '今後は、伝えられた言葉を一つずつ確認して丁寧に送ってほしいことを、責める口調にならないよう柔らかく伝えてください。',
          '',
          '不足していた具体的な言葉をユーザーへ再送するよう要求してはいけません。',
          '新しい質問や新しい工程も作らないでください。',
          '',
          'この返信の後には本来の次の鑑定文章が続くため、最後は自然に続きを進める形で締めてください。'
        ].join('\n');
        } else if (
          itemReason.includes('ユーザーメッセージ通数(') &&
          itemReason.includes('< span個数(')
          ...
          generationInstruction = [
            `必要な工程が${Number(item.spanShortage || 0)}個不足しているケースです。`,
            '',
            '今回は不足数が多いため、鑑定を先へ進めてはいけません。',
            '鑑定士から先ほど伝えた工程が十分に完了していないことを自然に説明してください。',
            '',
            'ユーザーには、先ほど伝えた内容をもう一度確認し、未完了の工程を行ったうえで改めて連絡するよう促してください。',
            '',
            'ただし、直前の鑑定士本文に明記されていない工程名、返信ワード、順番、効果を新しく作ってはいけません。',
            'プロフィールに存在していても、直前の鑑定士本文に出ていない具体的な工程を勝手に持ち出してはいけません。',
            '不足している工程をAIが推測して特定してはいけません。',
            '',
            '新しい質問や追加の鑑定展開は作らないでください。'
          ].join('\n');
      ...
      generationInstruction = [
        'ユーザーが送り返した言葉の中に、漢字の誤り・誤字があるケースです。',
        '',
        '鑑定士本人からユーザーへ直接伝える、柔らかく自然な文章にしてください。',
        '審査結果やシステム判定を通知しているような堅い文章にしてはいけません。',
        '',
        '「表記に一部違いが見られました」',
        '「意図を汲んで受け取ります」',
        '「誤記の可能性があります」',
        ...
        'などの事務的・機械的な表現は禁止です。',
        '',
        '「先ほどお送り頂いた言葉の中に、漢字の間違いがありました」のように、',
        '普通の会話として自然に伝えてください。',
        '',
        '送り返す言葉は一文字でも違うと、正しく力を受け渡せない可能性があるため、',
        '次からは正しい言葉を確認して送るよう気を付けてほしいことを柔らかく伝えてください。',
        '',
        '今回の誤った言葉を正しいものとして受理した、許容した、認めた、',
        '意図を汲んだので問題ない、という意味の文章は絶対に作らないでください。',
        '',
        '今回先へ進むことになっていても、誤字そのものを正しいものとして扱ったとは説明しないでください。',
        '',
        'ユーザーを責めたり強く叱ったりせず、次回から気を付けてもらう程度の口調にしてください。',
        'この後に本来の次の鑑定文章が続くため、簡潔にしてください。',
