...
function extractNickname(userTexts) {
  ...
  function resolveNickname(candidate) {
    ...
    // 「ちゃん」はニックネームの一部としてそのまま保存
    // 例：さっちゃん / ばばちゃん / ケンちゃん
    if (
      /^[一-龥々ぁ-んァ-ヶーA-Za-z0-9]{1,10}ちゃん$/.test(
        candidate
      )
    ) {
      return candidate;
    ...
    // 「さん」は最終表示側で「さん」が付くため除去
    // 例：やっさん → やっ
    if (
      /^[一-龥々ぁ-んァ-ヶーA-Za-z0-9]{1,10}さん$/.test(
        candidate
      )
    ) {
      const base =
        candidate.slice(0, -2).trim();
  ...
  const explicitNicknamePatterns = [
    ...
    // 名前は美月です。最近～
    /(?:私の名前は|(?:^|\n)\s*名前は)([一-龥々ぁ-んァ-ヶーa-zA-Z0-9]{1,12}?)(?:です(?:けど|が|けれど|けれども)?)?(?=[。、！!\s]|$)/m,
  ...
  const nameWaM = text.match(
    /(?:^|\n)(?:私の)?名前は([一-龥々ぁ-んァ-ヶーa-zA-Z0-9]{2,12}?)(?:です|と申します|といいます|と言います)[。！!\s]*(?:\n|$)/
  );
  if (nameWaM) {
  ...
  const watashiM = text.match(
    /(?:^|\n)私は([一-龥々ぁ-んァ-ヶーa-zA-Z0-9]{2,12}?)(?:です|と申します|といいます|と言います)[。！!\s]*(?:\n|$)/
  );
            ...
            content: [
              {
                ...
                text: [
                  ...
                  '',
                  '目的は、今後ユーザーを自然に呼びかける際に使用する名前・呼び名を取得することです。',
                  '名（下の名前）が分かる場合は名を優先し、名が分からず姓しか確認できない場合は姓を採用します。',
                  '',
                  '【絶対ルール】',
                  '・本人が明示したニックネームや呼び名がある場合は、それを最優先する',
                  '・「ちゃん」で終わる呼び名は、「ちゃん」を含めたまま返す',
                  '・例: さっちゃん → さっちゃん',
                  '・例: ばばちゃん → ばばちゃん',
                  '・「さん」で終わる呼び名は、末尾の「さん」を除いて返す',
                  '・例: やっさん → やっ',
                  '・フルネームが分かる場合は原則として名（下の名前）を優先する',
                  '・フルネームをそのままgivenNameにしない',
                  '・名が分からず姓しか確認できない場合は、姓を呼び名として採用してよい',
                  '・本文に存在しない名前を創作しない',
                  ...
                  '・ユーザー本人の氏名・名前・呼び名として書かれているものだけを対象にする',
                  '',
                  'confidenceは次の基準で返す',
                  'high: ユーザー本人の名前・呼び名と十分明確に判断できる。名が分かる場合は名を優先し、名が不明でも姓が本人名として明確ならhigh',
                  'medium: 本人の名前候補ではあるが、他人名・地名・一般語など別の可能性が残る',
                  'low: 推測が大きい',
                  '',
                  '姓しか確認できないこと自体はconfidenceを下げる理由にしないでください。',
                  '本人の姓であることが十分明確ならconfidence=highにしてください。',
                  '自動保存に使うため、本人の名前かどうか迷う場合だけmediumまたはlowにしてください。'
                ].join('\n')
              }
            ]
          },
...
function validateStructuredYesNo(text, rule) {
  ...
  // 明確な肯定
  if (
    /^(?:はい[、,\s]*(?:あります|ある|ありました)?|あります|ある|ありました|一度あります|何度かあります)[。！!\s]*$/.test(t)
  ) {
    return {
      status: 'valid',
      reason: 'yes_answer'
  ...
  // 明確な否定
  if (
    /^(?:いいえ[、,\s]*(?:ありません|ない|ないです|ありませんでした)?|ありません|ない|ないです|ありませんでした)[。！!\s]*$/.test(t)
  ) {
    return {
      status: 'valid',
      reason: 'no_answer'
          ...
          content: [
            {
              ...
              text: [
                ...
                '',
                '【valid_answer の重要ルール】',
                'valid_answer は、ユーザーが質問で求められている情報そのものを実際に提示している場合だけ選択してください。',
                '質問に関連する話題を話しているだけでは valid_answer にしてはいけません。',
                'ユーザーが明言していない回答を、文脈や状況から推測して補ってはいけません。',
                '例：周期的な出来事を聞かれて「冬は寒いので苦手です」だけなら valid_answer ではありません。',
                '例：部屋に特定の物があるか聞かれて「部屋は片付いています」だけなら valid_answer ではありません。',
                '判断できない場合は other を選択してください。',
                '',
                '【拒否カテゴリの重要ルール】',
                '拒否・否定表現が含まれているだけで refusal 系カテゴリを選んではいけません。',
                '何を拒否しているのかを必ず確認してください。',
                'カテゴリが「名前を伝えることへの拒否」であれば、名前・本名・呼び名・ニックネーム・イニシャルを教えたくないという意味が明確な場合だけ選択してください。',
                '鑑定継続、有料利用、ポイント購入、サービス利用、別の質問などを拒否している場合は、その refusal カテゴリには該当しません。',
                '該当カテゴリがなければ other を選択してください。',
                '',
...
function buildSkippedGenerationInstruction(
  item,
  ai,
  itemReason
) {
...
async function testExcludedReply({
  ...
  const item = {
    ...
    expectedReplyWords:
      Array.isArray(expectedReplyWords)
        ? expectedReplyWords
        : [],

    unmatchedWords:
      Array.isArray(unmatchedWords)
        ? unmatchedWords
        : [],
